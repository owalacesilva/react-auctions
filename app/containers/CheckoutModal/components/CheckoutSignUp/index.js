...
function CheckoutSignUp({ onChangeMode, onSubmit }) {
  ...
  const handleDocumentCpf = event => {
    ...
    if (value.length <= 3) {
      ...
      setDocumentCpf(
        value.replace(/(\d{3})(\d{3})(\d{3})(\d{0,2})/, '$1.$2.$3-$4'),
      );
    }
  };
  ...
  return (
    <div className="mb-3">
      <div className="alert alert-warning p-2 font-xss mb-1">
        <i className="bi bi-exclamation-circle" /> Informe os dados corretos
        para recebimento das premiações.
      </div>
      <Form
        name="login"
      ...
      >
        ...
        <Form.Group className="mb-2">
          <p className="app-termos-text text-muted font-xs mb-0">
            Ao realizar este pagamento e confirmar minha participação nesta ação
            entre amigos, declaro ter lido e concordado com os{' '}
            <a
